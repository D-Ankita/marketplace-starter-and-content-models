...
export class MarketplaceStarter {
  ...
  async starterDetails() {
    ...
    this.starterAppName =
      process.env.CONTENTSTACK_STARTER_APP?.toLowerCase().replace(/ /g, "-") + "-" + Math.floor(Math.random() * 10);
  ...
  async vercelDeploy() {
    const deployButton = await this.page.waitForSelector('button:has-text("Deploy in Vercel")');
  ...
  async vercelDeployment() {
    ...
    await popup.close(); // remove after fix is implemented
    // following comment is for future implementation
  ...
  async deleteStack() {
    ...
    let ele = await this.page.locator(`[data-test-id="cs-link"] >> text="${this.starterAppName}"`);
    ele.click();
    await this.page.locator('[name="Setting"]').click();
    await this.page.locator('[data-test-id="cs-delete-stack"] >> text="Delete Stack"').click();
    await this.page.locator('.Button--delete >> text="Delete"').click();
  }
}
